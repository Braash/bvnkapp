...
const PaymentProcessView: React.FC<PaymentProcessViewProps> = ({ selectedCurrency, handleCurrencyChange, handleAcceptedQuote, handleUpdatedQuote, paymentSummaryData, confirmPaymentSummaryLoading, paymentSummaryDataLoading, uuid }) => {
  return (
    <div className="flex justify-center items-center h-screen bg-gray-100">
      <div className="w-1/2 bg-white rounded-lg shadow-lg p-8 relative">
        ...
        <h2 className="text-center text-gray-500 md:text-sm text-xs mb-4">{messages?.reference} <span>{response?.reference}</span></h2>
        <div className="relative">
        <h6 className="mb-1 text-xs font-semibold">{messages?.payWithLabel}</h6>
          <select
        ...
        {selectedCurrency && (
            ...
            <div className="border-t border-gray-300 top-0 w-full left-0 h-px bg-gray-300 mt-2 mb-2" />
				...
				<div className="flex justify-between">
					<div className="text-center text-gray-500 md:text-sm text-xs">{messages?.amountDueLabel}</div>
					<div>
            ...
            <div className="border-t border-gray-300 top-0 w-full left-0 h-px bg-gray-300 mt-2 mb-2" />
				...
				<div className="flex justify-between">
					<div className="text-center text-gray-500 md:text-sm text-xs">{messages?.expirationCopy}</div>
					<div>
						{paymentSummaryDataLoading ? <img
							src={loaderImg}
							className="h-6 object-cover bg-transparent"
          ...
          >
            {confirmPaymentSummaryLoading ? messages?.processingCopy : messages?.confirmCopy}
          </button>
        </div>
        )}
      </div>
